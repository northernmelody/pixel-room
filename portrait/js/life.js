...
// Evening blocks are the portrait equivalent of the horizontal build's night leisure window.
const EVENING_LEISURE=new Set(['sofa','game','read','exercise','playCat','lookOut','phone','snack','change','guitar','swing','drink']);
...
export class LifeDirector{
  ...
  react(kind=''){const choices=kind==='lookup'?['抬头看了看灯','抬头望了望天花板']:this.pose==='sleep'?['翻了个身','睡梦里轻轻呼吸']:this.pose==='shower'?['隔着水声哼了一句歌','冲你眨了眨眼']:['朝你挥了挥手','笑着点点头','回头看了你一眼','有点惊喜'];this.reaction={text:choices[Math.floor(Math.random()*choices.length)],timer:1.6};this.onChange(this.snapshot());}
  blocked(){if(this.schedule==='sleep'||this.activity==='sleep'||this.pose==='sleep')return 'sleep';if(this.schedule==='call'||this.activity==='call')return 'call';return '';}
  ...
  startChange(){if(this.activity==='change')return 'busy';const blocked=this.blocked();if(blocked)return blocked;if(!EVENING_LEISURE.has(this.schedule))return 'later';this.setPlan('change');return 'ok';}
  snapshot(){const step=this.current();const elapsed=Math.max(0,(step?.duration||0)-this.timer);const call=this.schedule==='call'?STORY.calls[dayNumber()%STORY.calls.length]:null;let lyric='';if(this.pose==='guitar'&&this.song){const i=Math.min(this.song.lyrics.length-1,Math.floor(elapsed/this.song.tempo));lyric=this.song.lyrics[i]||'';}const callLine=call?call.lines[Math.min(call.lines.length-1,Math.floor(elapsed/150))]||null:null;return {schedule:this.schedule,activity:this.activity,title:TITLES[this.activity]||TITLES[this.schedule]||'平凡的一天',label:this.label,pose:this.pose,held:this.held,motion:this.motion,meal:this.meal,snack:this.snack,song:this.song,lyric,call,callLine,reaction:this.reaction?.text||'',step:this.index};}
