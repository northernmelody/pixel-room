...
import { COUPLE_ACTIVITY, resolveCoupleState } from './couple.js';

const anchor=Object.fromEntries(ANCHORS.map(a=>[a.id,a]));
const TITLES={sleep:'睡得很安稳',wakeUp:'慢慢醒来',dress:'换上今天的衣服',morning:'晨间洗漱',morningBuffer:'准备开始一天',breakfast:'准备早餐',lunch:'准备午餐',dinner:'准备晚餐',work:'认真工作',sofa:'在沙发上放空',leisure:'晚间休闲',shower:'洗个热水澡',undress:'收起白天的衣服',game:'打会儿游戏',read:'在床边看书',exercise:'舒展身体',playCat:'陪猫玩一会儿',lookOut:'看看窗外',snack:'拿一点零食',change:'换一身衣服',guitar:'弹一首歌',swing:'去院子荡秋千',drink:'喝一点水'};
...
function dailyBlocks(){
  ...
  const evening={
    1:[['sofa',40],['playCat',30],['read',40],['lookOut',30],['read',40],['shower',30],['undress',30]],
    2:[['sofa',30],['exercise',45],['shower',30],['drink',15],['read',75],['undress',25]],
    3:[['guitar',55],['read',40],['playCat',30],['swing',25],['snack',25],['read',30],['shower',30],['undress',25]],
    4:[['game',55],['playCat',30],['read',40],['lookOut',25],['shower',30],['undress',25]],
    5:[['game',75],['snack',30],['guitar',55],['playCat',30],['read',40],['lookOut',30],['shower',30],['undress',25]],
    6:[['game',60],['swing',30],['read',45],['guitar',45],['snack',30],['lookOut',35],['shower',30],['undress',30]],
    0:[['playCat',35],['read',55],['lookOut',70],['shower',30],['undress',25]]
  }[dow];
...
export class LifeDirector{
  ...
  setPlan(activity){
    ...
    else this.steps=couplePlans[activity]||plans[activity]||plans.sleep;this.beginCurrent();
  }
