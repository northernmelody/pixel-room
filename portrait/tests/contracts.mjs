...
import { resolveCoupleState, GIRLFRIEND_PRESENCE, COUPLE_ACTIVITY, BED_STATE, BEDROOM_LIGHT, VISIT_WINDOWS, BEDTIMES } from '../js/couple.js';
...
const boundaries=[['07:19:55','07:20:00','SLEEP_TOGETHER','WAKE_TOGETHER'],['07:34:55','07:35:00','wakeUp','dress'],['07:44:55','07:45:00','dress','morning'],['08:04:55','08:05:00','morning','breakfast'],['08:34:55','08:35:00','breakfast','swing'],['08:59:55','09:00:00','morningBuffer','work'],['11:59:55','12:00:00','work','lunch'],['12:59:55','13:00:00','lunch','work'],['17:59:55','18:00:00','work','dinner'],['18:59:55','19:00:00','dinner','ARRIVING']];
...
for(const [time,kind,pose] of [['07:45:00','morning','brush'],['08:05:00','breakfast','eat'],['12:00:00','lunch','eat'],['18:00:00','dinner','eat'],['19:15:00','couple','coupleSnack']]){at(time);const nav=makeNav(),life=new LifeDirector(nav,world),seen=new Set();for(let i=0;i<1600;i++){seen.add(life.pose);const prior=stable(life,nav);life.react();assert.equal(stable(life,nav),prior,'Human click never changes route, pose, step or timer');nav.update(.05);life.update(.05);}assert.ok(seen.has(pose),kind);if(pose==='eat')for(const p of ['fridge','cook','eat'])assert.ok(seen.has(p));if(kind==='morning')for(const p of ['toilet','flush','washHands','brush'])assert.ok(seen.has(p));}
...
for(const [date,time,expected] of [['2026-09-14','20:00:00','WATCH_TV'],['2026-09-15','20:00:00','SWITCH_COOP'],['2026-09-16','20:00:00','FIREPLACE_CHAT'],['2026-09-18','22:30:00','FIREPLACE_CHAT'],['2026-09-19','09:00:00','morning'],['2026-09-20','20:00:00','FIREPLACE_CHAT']]){at(time,date);const first=new LifeDirector(makeNav(),world).schedule;const second=new LifeDirector(makeNav(),world).schedule;assert.equal(first,expected,date+' profile');assert.equal(second,first,'Profile is deterministic');}
// Full leisure loops and forced schedule interruption use the real state machine.
at('19:00:00');
for(const activity of ['game','read','exercise','playCat','lookOut','snack','change','guitar','swing']){const nav=makeNav(),life=new LifeDirector(nav,world);life.setPlan(activity);let seen=new Set(),visited=[];for(let i=0;i<9000&&life.activity===activity;i++){seen.add(life.pose);visited.push([nav.x,nav.y]);nav.update(.05);life.update(.05);}assert.ok(seen.size>=2,activity);if(activity==='swing'){assert.ok(visited.some(([x,y])=>x===156&&y>346&&y<GARDEN.pathY),'Use the garden gate');assert.ok(visited.some(([x,y])=>x===GARDEN.swingX&&y===GARDEN.swingY),'Reach the swing');}life.setPlan(activity);step(nav,life,25);at('21:30:00');life.update(.05);assert.equal(life.schedule,COUPLE_ACTIVITY.IDLE_TOGETHER);assert.equal(life.song,null);assert.equal(life.held,'');at('19:00:00');}
...
// Girlfriend presence, weekly rotation, phone regression and shared-world determinism.
for(const [date,time,presence] of [['2026-09-14','14:00:00','AWAY'],['2026-09-14','20:00:00','VISITING'],['2026-09-14','21:31:00','VISITING'],['2026-09-19','19:00:00','VISITING'],['2026-09-20','03:00:00','STAYING_OVERNIGHT'],['2026-09-20','08:44:59','STAYING_OVERNIGHT'],['2026-09-20','08:45:00','AWAY']]){at(time,date);assert.equal(resolveCoupleState().presence,presence,date+' '+time);}
for(const [date,time,activity] of [['2026-09-14','20:00:00','WATCH_TV'],['2026-09-15','20:00:00','SWITCH_COOP'],['2026-09-16','20:00:00','FIREPLACE_CHAT'],['2026-09-18','21:30:00','WATCH_TV'],['2026-09-19','21:00:00','FIREPLACE_CHAT'],['2026-09-20','20:00:00','FIREPLACE_CHAT']]){at(time,date);const a=resolveCoupleState(),b=resolveCoupleState();assert.equal(a.activity,activity,date);assert.deepEqual(a,b,'Same Beijing timestamp has one shared state');}
assert.deepEqual(VISIT_WINDOWS.map(w=>[w.start,w.end]),[[1140,1275],[1140,1290],[1140,1320],[1140,1335],[1140,1305],[1140,1380],[1110,1410]]);
assert.deepEqual(BEDTIMES,[1385,1380,1380,1420,1380,1455,1475]);
for(const [date,time,activity,bedState,lightState] of [
  ['2026-09-14','22:45:00','BED_CHAT','BED_OCCUPIED_CHAT','WARM_BEDSIDE'],
  ['2026-09-15','22:45:00','CUDDLE','BED_OCCUPIED_CUDDLE','WARM_BEDSIDE'],
  ['2026-09-16','23:25:00','SETTLING_TO_SLEEP','BED_OCCUPIED_CUDDLE','DIM'],
  ['2026-09-17','22:50:05','KISS_GOODNIGHT','BED_OCCUPIED_CHAT','WARM_BEDSIDE'],
  ['2026-09-18','23:55:00','UNDER_BLANKET_INTIMACY','BED_OCCUPIED_INTIMATE','DIM'],
  ['2026-09-20','00:20:00','UNDER_BLANKET_INTIMACY','BED_OCCUPIED_INTIMATE','DIM'],
  ['2026-09-20','22:50:00','CUDDLE','BED_OCCUPIED_CUDDLE','WARM_BEDSIDE']
]){at(time,date);const couple=resolveCoupleState();assert.equal(couple.activity,activity,date+' '+time);assert.equal(couple.bedState,bedState);assert.equal(couple.lightState,lightState);}
for(const time of ['01:00:00','04:00:00']){at(time,'2026-09-15');const couple=resolveCoupleState();assert.equal(couple.presence,GIRLFRIEND_PRESENCE.STAYING_OVERNIGHT,'Direct overnight load');assert.equal(couple.activity,COUPLE_ACTIVITY.SLEEP_TOGETHER);assert.equal(couple.bedState,BED_STATE.OCCUPIED_SLEEP);assert.equal(couple.lightState,BEDROOM_LIGHT.SLEEP);}
at('07:22:00','2026-09-15');{const couple=resolveCoupleState();assert.equal(couple.activity,COUPLE_ACTIVITY.WAKE_TOGETHER);assert.equal(couple.bedState,BED_STATE.OCCUPIED_WAKE);}
at('07:36:00','2026-09-15');assert.equal(resolveCoupleState().presence,GIRLFRIEND_PRESENCE.AWAY,'Girlfriend leaves after the weekday morning departure');
for(let dow=14;dow<=20;dow++)for(let minute=18*60;minute<24*60;minute+=5){const date='2026-09-'+dow,time=String(Math.floor(minute/60)).padStart(2,'0')+':'+String(minute%60).padStart(2,'0')+':00';at(time,date);const life=new LifeDirector(makeNav(),world);assert.notEqual(life.schedule,'phone');assert.notEqual(life.schedule,'call');assert.equal(life.snapshot().call,null);}
...
// Manual guitar keeps the horizontal refusal rules; visits, washing and showering are off limits too.
at('20:00:00');{const nav2=makeNav(),life2=new LifeDirector(nav2,world);assert.equal(life2.schedule,'WATCH_TV');assert.equal(life2.startGuitar(),'visit');assert.equal(life2.schedule,'WATCH_TV','Manual guitar does not hijack the shared evening');}
...
at('21:50:00');{const life2=new LifeDirector(makeNav(),world);assert.equal(life2.schedule,COUPLE_ACTIVITY.IDLE_TOGETHER);assert.equal(life2.startGuitar(),'visit','The overnight visit cannot be interrupted');}
at('03:00:00');{const life2=new LifeDirector(makeNav(),world);life2.setPlan('sleep');assert.equal(life2.startGuitar(),'sleep');}
at('21:00:00','2026-09-14');{const nav2=makeNav(),life2=new LifeDirector(nav2,world);assert.equal(life2.schedule,'FIREPLACE_CHAT');assert.equal(life2.startGuitar(),'visit');assert.equal(life2.snapshot().girlfriend.presence,GIRLFRIEND_PRESENCE.VISITING);}
...
for(const pose of ['sleep','fridge','cook','eat','work','game','toilet','read','guitar','swing','washHands','brush','shower','exercise','wardrobe','change','door','coupleSit','coupleGame','coupleSnack','coupleRead','bedChat','cuddle','kissGoodnight','underBlanket','settlingSleep','sleepTogether','wakeTogether'])drawScene({getContext:()=>ctx},{...prefs,particles:true},{actor:{...nav.snapshot(),pose,motion:1,held:'controller'},life:{pose,motion:1},pets:{cat:{...highCat,motion:1,direction:1},dog:{kind:'dog',x:100,y:346,state:'sleep',motion:1}},world,weather:{kind:'storm'},time:{hour:23}});
at('14:00:00','2026-09-14');{const life2=new LifeDirector(makeNav(),world),result=drawScene({getContext:()=>ctx},{...prefs,particles:false},{actor:{...nav.snapshot(),pose:'idle',motion:1},life:life2.snapshot(),world,time:{hour:14}});assert.equal(result.girlfriendActors,0);}
at('20:00:00','2026-09-14');{const life2=new LifeDirector(makeNav(),world),result=drawScene({getContext:()=>ctx},{...prefs,particles:false},{actor:{...nav.snapshot(),pose:'idle',motion:1},life:life2.snapshot(),world,time:{hour:20}});assert.equal(result.girlfriendActors,1);assert.equal(result.maleActors,1);}
at('03:00:00','2026-09-15');{const life2=new LifeDirector(makeNav(),world),result=drawScene({getContext:()=>ctx},{...prefs,particles:false},{actor:{...nav.snapshot(),pose:'idle',motion:1},life:life2.snapshot(),world,time:{hour:3}});assert.equal(result.girlfriendActors,1);assert.equal(result.maleActors,1);assert.equal(result.bedState,BED_STATE.OCCUPIED_SLEEP);}
...
console.log('PASS S6 + M7D-02: autonomous schedule, deterministic overnight lifecycle, weekly couple bedtime rotation, shared bed/light states, refresh-safe sleep, morning departure, classic click scope, pets, garden routes, lamps, letters, storage, weather and drawing.');
