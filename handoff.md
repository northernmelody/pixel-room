...
- Vercel Production：`https://pixel-room-eight.vercel.app`，本轮第三次部署（deployment `https://pixel-room-11tvoclyn-northernmelodys-projects.vercel.app`，`READY`），线上包含横版入口修复、竖版点击范围收束、第二批 6 项需求与左上角「当前行为」说明条。
...
- 庭院新增两只像素玩偶：秋千左侧的大象（18×15）与秋千、踏石之间的长颈鹿（15×24），程序化绘制、随昼夜调色、带落地阴影，均为纯场景装饰（不可点击、无悬浮）；契约测试断言摆位避开秋千支架、两段踏石与四条花坛。
...
线上验收（部署后）：`/`、`/portrait/`、`/classic/`、`/setting/`、`/portrait/js/main.js`、`/portrait/style.css` 全部返回 200；`main.js`、`style.css`、`portrait/index.html`、`scene.js` 的线上内容与提交内容逐字节一致（LF 归一化）。生产域名真实浏览器验收：多视口回归 5 档 ALL PASS，线上说明条显示「秋日 · 阳光正好 · 走到电脑前」且时钟在画面内、零页面错误（`node portrait/tools/viewport-check.cjs https://pixel-room-eight.vercel.app/portrait/?d=2026-09-17&t=16:30`）；横版入口 ALL PASS，截图 `_shots/classic-prod-check.png`。
...
横版场景的可点击范围由 `js/interaction.js`、`js/lifeUI.js` 和 `js/roomLayout.js` 共同定义，当前保留以下交互：

- 小人：点击触发短暂回应，不改变作息。
- 猫：点击摸猫；狗：首次点击吠叫，短时间内再次点击会跟随小人。
- 吉他：点击触发取琴、弹唱、归还流程；睡觉、通话或正在弹唱时不可重复触发。
- 电脑：点击打开电脑放大屏，弹窗内可换台、关闭或点击外部关闭。
- 4 盏顶灯、工作区台灯、卧室床头灯：点击开关；快速连续切换会触发猫被吓跑的反馈。
- 墙面玩偶和床上腊肠狗抱枕：点击显示名称提示。
- 书架两封信：点击阅读给 MOMO 的信和 MOMO 的回信。
- 厨房橱柜、台面、餐桌碗：做饭或吃饭时显示当前食物，否则提示餐桌为空。
- 衣柜：晚间休闲时可触发换衣，其他时间仅显示提示；户外木门目前只显示未开放提示。
- 通话期间的 MOMO 卡片：点击查看完整通话记录。
...
- 灯具光重做：由"大半径加法光晕（环境雾）"改为"灯珠 + 表面光池"——顶灯在正下方地面/家具形成压扁椭圆光池，台灯在桌面、床头灯在床头柜形成局部暖光池；灯珠仅 2px，消除过曝白球。
- 四盏顶灯独立配置：半径/亮度/色温各不相同（卧室暖黄 r34、工作区中性 r38、卫生间冷白 r30、厨房暖黄 r36），每间房独立照明，房间间隔墙不再被光雾淹没。
- 顶灯照明范围覆盖整间房：光池半径 ≥ 半房间宽（中心在地面、向上覆盖家具与人物全身，房间边缘的宠物/人物也在光内）+ 一层从灯辐射的房间泛光（照亮墙面/书架/柜顶），开灯后整间房有整体亮度变化而非仅灯与地板变亮。
- 墙面柔光缩小为灯具附近的局部微弱光（提供受光反馈但不成雾），地面/桌面/床面光池成为主要受光反馈。
- 光源主次：白天以窗户光为主、灯默认关闭；夜间以顶灯为主、台灯/床头灯为局部辅光。
- 窗户光照：墙体光晕中心下移（光从窗口洒向下方墙面与地面，而非对称环状），亮度再降；窗内太阳/月亮进一步缩小变淡（日轮 hSpan×0.11、光晕更紧），暮色天空降饱和、暖色集中在窗底，并按每扇窗采光微差。
- 窗内内容统一裁剪在玻璃范围内，不再溢出窗框与窗台；墙体光晕挖掉玻璃区域，与窗框/中梃遮挡关系更自然。
